...
export function validateVideoBatch(
  ...
  const errors: string[] = [];
  const accepted: File[] = [];
  let remaining = MAX_VIDEOS - existingCount;

  if (remaining <= 0) {
    errors.push(`Only ${MAX_VIDEOS} video allowed per draft.`);
    return { accepted, errors };
  }

  for (const file of incoming) {
    if (remaining <= 0) {
      errors.push(`Only ${MAX_VIDEOS} video allowed — "${file.name}" skipped.`);
      continue;
    }
    ...
    if (
      ...
      errors.push(
        `"${file.name}" uses an unsupported format. Use MP4, MOV, or WebM.`,
      );
      continue;
    }
    if (file.size === 0) {
    ...
    if (file.size > MAX_VIDEO_BYTES) {
      errors.push(
        `"${file.name}" is ${formatBytes(file.size)} — max is ${formatBytes(MAX_VIDEO_BYTES)}.`,
      );
      continue;
    }
    accepted.push(file);
